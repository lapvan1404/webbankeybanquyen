...
export class OrderService {
  ...
  public async payOrder(userId: string, id: string, customerEmail?: string): Promise<OrderDto> {
    ...
    // Deliver email with keys to user's Gmail
    try {
      const keys = await this.getOrderLicenseKeys(userId, order.id);
      const user = await this.prisma.user.findUnique({ where: { id: order.userId } });
      const targetEmail = customerEmail?.trim() || user?.email || 'khachhang@gmail.com';
      await this.emailService.sendLicenseKeysEmail(
        targetEmail,
        order.orderNumber,
        Number(order.totalAmount),
        keys.map((k) => ({ productName: k.productName, key: k.key })),
      );
    } catch (err) {
  ...
  public async handleBankWebhook(rawContent: string): Promise<OrderDto | null> {
    try {
      ...
      return this.markOrderPaidAdmin(matchedOrder.id);
    } catch (err) {
  ...
  public async markOrderPaidAdmin(id: string): Promise<OrderDto> {
    const order = await this.repository.findByIdWithItems(id);
    if (!order || order.deletedAt) {
      throw new HttpError(404, 'Order not found.', 'Payment failed');
    }
    ...
    // Deliver email with keys to user's Gmail
    try {
      const keys = await this.getOrderLicenseKeys(order.userId, order.id);
      const user = await this.prisma.user.findUnique({ where: { id: order.userId } });
      const userEmail = user?.email || 'khachhang@gmail.com';
      await this.emailService.sendLicenseKeysEmail(
        userEmail,
        order.orderNumber,
        Number(order.totalAmount),
