...
const slides: Array<{ image?: string; title?: string }> = [
  { title: "Ưu đãi phần mềm chính hãng" },
  { title: "Flash sale key bản quyền" },
...
function Index() {
  ...
  const validBanners = (bannersQuery.data ?? []).filter(
    (b) => b.imageUrl && !b.imageUrl.includes("example.com"),
  );
  const bannerSlides = validBanners.length
    ? validBanners.map((banner) => ({ image: banner.imageUrl, title: banner.title }))
    : slides;
