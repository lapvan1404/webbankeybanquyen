...
type AdminUserDetail = {
  user: AdminUserItem & { totalOrders: number };
...
function AdminDashboard() {
  ...
  // States for Customer User Management
  ...
  const [userToToggleStatus, setUserToToggleStatus] = useState<AdminUserItem | null>(null);
  ...
  const [orderToConfirmPayment, setOrderToConfirmPayment] = useState<OrderData | null>(null);
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch<any>(`/api/admin/users?${query}`, { credentials: "include" });
      ...
      if (err?.status === 401) {
        ...
        setUsersError("Bạn không có quyền truy cập module này.");
      } else {
        setUsersError("Không thể tải danh sách tài khoản.");
      }
    } finally {
      setUsersLoading(false);
    }
  }, [userSearch, userStatusFilter, userPage, navigate]);
  ...
  const handleViewUserDetail = async (userId: string) => {
    ...
    try {
      const res = await apiFetch<any>(`/api/admin/users/${userId}`, { credentials: "include" });
      const payload = res?.user ? res : res?.data ? res.data : res;
      if (payload?.user) {
  ...
  const handleConfirmToggleUserStatus = async () => {
    if (!userToToggleStatus || mutationLoading) return;
    const targetStatus = userToToggleStatus.status === "ACTIVE" ? "LOCKED" : "ACTIVE";
    const actionText = targetStatus === "LOCKED" ? "khóa" : "mở khóa";

    setMutationLoading(true);
    try {
      const res = await apiFetch<any>(`/api/admin/users/${userToToggleStatus.id}/status`, {
        method: "PATCH",
        body: { status: targetStatus },
        credentials: "include",
      });

      toast.success(res.message || `Đã ${actionText} tài khoản thành công!`);
      ...
      toast.error(err.message || `Không thể ${actionText} tài khoản.`);
    } finally {
      setMutationLoading(false);
    }
  };

  const handleAdminConfirmPaymentSubmit = async () => {
    ...
    try {
      ...
      toast.error(err?.message || "Không thể xác nhận thanh toán đơn hàng.");
    } finally {
  ...
  return (
    <Layout hideHeaderFooter>
      ...
      <div className="max-w-[1536px] mx-auto px-4 lg:px-8 py-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 ring-1 ring-zinc-200 shadow-sm space-y-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-0.5">
                    Quản lý, tìm kiếm, xem chi tiết và khóa/mở khóa tài khoản trực tiếp từ Database
                  </p>
                </div>
                ...
                <div className="relative w-full md:w-80">
                  <input
                    ...
                    className="bg-zinc-50 ring-1 ring-zinc-200 focus:ring-brand rounded-full px-4 py-2.5 text-xs w-full outline-none transition pr-10"
                  />
                  {userSearch && (
                    <button
                      onClick={() => {
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-zinc-100">
                <div className="flex flex-wrap gap-2">
                  {[
                    { key: "ALL", label: `Tất cả (${userCounts.total})` },
                    { key: "ACTIVE", label: `🟢 Hoạt động (${userCounts.active})` },
                    { key: "LOCKED", label: `🔴 Đã khóa (${userCounts.locked})` },
                  ].map((tab) => (
                    <button
                      key={tab.key}
                      onClick={() => {
                        setUserStatusFilter(tab.key as any);
                        setUserPage(1);
                      }}
                      className={`px-4 py-1.5 rounded-full text-xs font-semibold transition cursor-pointer ${
                        userStatusFilter === tab.key
                          ? "bg-[#35B7BC] text-white shadow-md shadow-[#35B7BC]/20"
                          : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"
                      }`}
                    >
                      {tab.label}
            ...
            {usersLoading ? (
              ...
              <div className="bg-red-50 rounded-3xl p-8 ring-1 ring-red-200 text-center space-y-2">
                <p className="text-sm font-bold text-red-600">⚠️ {usersError}</p>
                <button
                  onClick={loadUsers}
                  className="text-xs font-semibold px-4 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 transition cursor-pointer"
                >
                  Thử lại
                </button>
              </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-100 text-zinc-800 font-medium">
                      {adminUsers.map((u, idx) => {
                        const stt = (userPagination.page - 1) * userPagination.limit + idx + 1;
                        const isLocked = u.status === "LOCKED";
                        const isAdminAccount = u.role?.toLowerCase() === "admin";
                            ...
                            <td className="py-3.5 px-4 text-center">
                              <div className="flex items-center justify-center gap-2">
                                <button
                                  ...
                                  className="px-2.5 py-1.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-xs font-semibold flex items-center gap-1 transition cursor-pointer"
                ...
                <div className="bg-white rounded-3xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                  <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-zinc-50">
                    ...
                    <button
                      ...
                      className="size-8 rounded-full bg-zinc-200 hover:bg-zinc-300 grid place-items-center text-zinc-600 transition cursor-pointer"
                    >
                      ✕
                    </button>
                  </div>
                  ...
                  <div className="p-6 overflow-y-auto space-y-6">
                    {detailLoading || !selectedUserDetail ? (
                      ...
                      <>
                        ...
                        {/* Order History */}
                        <div>
                          <h4 className="text-sm font-bold text-zinc-900 mb-3 flex items-center gap-1.5">
                            <span>🛒</span> LỊCH SỬ ĐƠN HÀNG ({selectedUserDetail.orders.length})
                          </h4>

                          {selectedUserDetail.orders.length === 0 ? (
                            <div className="bg-zinc-50 rounded-2xl p-6 text-center text-xs text-zinc-500">
                              Tài khoản này chưa phát sinh đơn hàng nào.
                            </div>
                          ) : (
                            <div className="ring-1 ring-zinc-200 rounded-2xl overflow-hidden">
                              <table className="w-full text-left text-xs border-collapse">
                                <thead>
                                  <tr className="bg-zinc-100 text-zinc-600 font-bold uppercase tracking-wider">
                                    ...
                                    <th className="py-2.5 px-3 text-center">Thanh toán</th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-zinc-100 font-medium text-zinc-800">
                                  {selectedUserDetail.orders.map((ord) => (
                                    <tr key={ord.id} className="hover:bg-zinc-50">
                                      <td className="py-2.5 px-3 font-mono font-bold text-[#35B7BC]">
                                        #{ord.orderNumber}
                                      </td>
                                      ...
                                      <td className="py-2.5 px-3 text-center">
                                        {ord.paymentStatus === "PAID" || ord.status === "PAID" ? (
            ...
            {isLockConfirmModalOpen && userToToggleStatus && (
                ...
                <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-6 space-y-5 border border-zinc-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">
                        {userToToggleStatus.status === "ACTIVE"
                          ? "Khách hàng bị khóa sẽ không thể đăng nhập hoặc thực hiện mua hàng."
                          : "Khách hàng được mở khóa sẽ có thể đăng nhập lại bình thường."}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2 border-t border-zinc-100">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl text-xs font-semibold border border-zinc-200 bg-white hover:bg-zinc-100 transition disabled:opacity-50 cursor-pointer"
                    >
                      Hủy
                    </button>
                    ...
                    {orders
                      ...
                      .map((order) => (
                        ...
                        >
                          ...
                          <td className="py-3.5 px-4">
                            {["paid", "approved"].includes(order.status?.toLowerCase()) ? (
                              ...
                              <span className="px-2.5 py-1 rounded-full font-bold text-[11px] bg-red-100 text-red-800 border border-red-200">
                                🔴 Đã hủy
                              </span>
                            ) : (
                              <span className="px-2.5 py-1 rounded-full font-bold text-[11px] bg-amber-100 text-amber-900 border border-amber-300">
            ...
            <div className="bg-white rounded-3xl max-w-2xl w-full p-6 md:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in duration-150">
              ...
              <div className="flex flex-wrap items-center justify-end gap-3 border-t pt-4">
                {modalOrder.status !== "approved" && modalOrder.status !== "paid" && (
                  <button
                    onClick={() => {
                      setOrderToConfirmPayment(modalOrder);
                      setIsConfirmPaymentModalOpen(true);
                    }}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-2.5 rounded-xl text-xs transition shadow-md cursor-pointer flex items-center gap-1.5"
                  >
        ...
        {isConfirmPaymentModalOpen && orderToConfirmPayment && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-6 space-y-5 border border-zinc-100">
              <div className="flex items-center gap-3">
                <div className="size-12 rounded-2xl bg-emerald-100 text-emerald-700 grid place-items-center text-xl shrink-0 font-bold">
                  ✓
                </div>
                <div>
                  <h3 className="text-lg font-bold text-zinc-900">
                  ...
                  <p className="text-xs text-zinc-500">
                    Bạn có chắc chắn giao dịch này đã nhận đủ tiền chuyển khoản?
                  </p>
                </div>
              </div>
              ...
              <div className="bg-zinc-50 p-4 rounded-2xl ring-1 ring-zinc-200 text-xs space-y-2 font-medium">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-zinc-800">{orderToConfirmPayment.email}</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="flex items-center justify-end gap-3 pt-2 border-t border-zinc-100">
                ...
                <button
                  disabled={confirmPaymentLoading}
                  onClick={handleAdminConfirmPaymentSubmit}
                  className="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition flex items-center gap-1.5 shadow-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
