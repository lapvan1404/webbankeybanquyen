...
export function CheckoutPage() {
  ...
  const placeOrder = async () => {
    ...
    if (!email.trim() || !phone.trim()) {
      toast.error("Vui lòng điền đầy đủ Email và Số điện thoại nhận key");
      return;
    }
    ...
    try {
      ...
      setOrder(created);
      toast.success("Tạo đơn hàng thành công! Hãy chọn ứng dụng và quét mã để thanh toán.");
    } catch (error) {
  ...
  const confirmPaid = async () => {
    ...
    setVerifyingStep(1);
    try {
      // Step 1: Connect to Banking API
      await new Promise((r) => setTimeout(r, 900));
      setVerifyingStep(2);

      // Step 2: Check bank statement for order code
      await payOrder(order.id, email);
      await new Promise((r) => setTimeout(r, 900));
      setVerifyingStep(3);

      // Step 3: Success & issue keys
      setIsPaidVerified(true);
      if (!isBuyNow) clear();
      toast.success("✅ ĐỐI SOÁT GIAO DỊCH NGÂN HÀNG MB BANK THÀNH CÔNG!");
      setTimeout(() => {
        window.location.href = `/checkout/success?orderId=${order.id}`;
      }, 1200);
    } catch (error) {
      setVerifyingStep(0);
      toast.error(error instanceof ApiError ? error.message : "Xác nhận đối soát ngân hàng thất bại");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-[1536px] mx-auto px-4 lg:px-8 py-8">
        ...
        {!order ? (
            ...
            <main className="col-span-12 lg:col-span-7 space-y-6">
              ...
              <div className="bg-white rounded-2xl ring-1 ring-black/5 p-6 space-y-4 shadow-sm">
                <h2 className="font-semibold text-lg text-zinc-900 flex items-center gap-2">
                  <span className="size-6 rounded-full bg-brand/10 text-brand text-xs grid place-items-center font-bold">1</span>
                  Thông tin nhận key bản quyền
                </h2>
                <Field label="Email nhận key bản quyền" type="email" placeholder="VD: email@gmail.com" value={email} onChange={setEmail} />
                <Field label="Số điện thoại liên hệ" type="tel" placeholder="VD: 0987654321" value={phone} onChange={setPhone} />
              </div>

              {/* Chọn phương thức thanh toán */}
              <div className="bg-white rounded-2xl ring-1 ring-black/5 p-6 space-y-4 shadow-sm">
                <h2 className="font-semibold text-lg text-zinc-900 flex items-center gap-2">
                  <span className="size-6 rounded-full bg-brand/10 text-brand text-xs grid place-items-center font-bold">2</span>
                  Chọn phương thức thanh toán
                </h2>
          ...
          <div className="max-w-2xl mx-auto bg-white rounded-2xl ring-1 ring-black/10 p-6 lg:p-8 shadow-xl space-y-6">
            ...
            {/* Trạng thái xác nhận & Kiểm tra ngân hàng */}
            {verifyingStep > 0 && !isPaidVerified && (
              <div className="bg-amber-50 border-2 border-amber-300 rounded-xl p-4 space-y-3 animate-pulse">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-amber-900 uppercase tracking-wider flex items-center gap-1.5">
                    <Loader2 className="size-4 animate-spin text-amber-600" />
                    Đang đối soát giao dịch MB Bank...
                  </span>
                  <span className="text-xs font-bold text-amber-800 bg-amber-200/80 px-2 py-0.5 rounded-full">
                    Bước {verifyingStep}/3
                  </span>
                </div>
                <div className="w-full bg-amber-200/60 h-2 rounded-full overflow-hidden">
                  <div
                    className="bg-emerald-600 h-full transition-all duration-500 rounded-full"
                    style={{ width: `${(verifyingStep / 3) * 100}%` }}
                  ></div>
                </div>
                <ul className="text-xs text-amber-950 space-y-1 pt-1 font-medium">
                  <li className="flex items-center gap-2">
                    <span className={verifyingStep >= 1 ? "text-emerald-700 font-bold" : "text-zinc-400"}>
                      {verifyingStep >= 1 ? "✓" : "○"} 1. Kết nối API hệ thống Ngân hàng MB Bank
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className={verifyingStep >= 2 ? "text-emerald-700 font-bold" : "text-zinc-400"}>
                      {verifyingStep >= 2 ? "✓" : "○"} 2. Kiểm tra sao kê với nội dung `CK {order.orderNumber}`
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className={verifyingStep >= 3 ? "text-emerald-700 font-bold" : "text-zinc-400"}>
                      {verifyingStep >= 3 ? "✓" : "○"} 3. Khớp số tiền {money(Number(order.totalAmount || finalTotal))} & Cấp key
                    </span>
                  </li>
                </ul>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`size-9 rounded-full text-white grid place-items-center shrink-0 transition-all ${
                    isPaidVerified ? "bg-emerald-600 ring-4 ring-emerald-200" : "bg-emerald-600"
                  }`}
                >
                  {isPaidVerified ? (
                ...
                <div className="text-xs">
                  <p className="font-bold text-emerald-950 text-sm">
                    {isPaidVerified
                      ? "✓ ĐÃ ĐỐI SOÁT XÁC NHẬN NHẬN TIỀN THÀNH CÔNG!"
                      : "Hệ thống đang chờ đối soát ngân hàng..."}
                  </p>
                  <p className="text-emerald-700">
                    {isPaidVerified
                      ? "Key bản quyền đã được kích hoạt và gửi tới Email của bạn. Đang chuyển hướng..."
                      : "Bấm 'Tôi đã thanh toán thành công' để hệ thống tiến hành đối soát giao dịch với Ngân Hàng."}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                className={`w-full font-bold py-4 px-6 rounded-xl transition-all shadow-lg text-base flex items-center justify-center gap-2 ${
                  isPaidVerified
              ...
              >
                {submitting ? (
                  <>
                    <Loader2 className="size-5 animate-spin" /> Đang đối soát với Ngân Hàng...
                  </>
                ) : isPaidVerified ? (
                  <>
                    <Check className="size-5 stroke-[3]" /> ✓ Đã đối soát thành công
                  </>
                ) : (
                  <>
                    <Check className="size-5" /> Tôi đã thanh toán thành công
                  </>
                )}
              </button>
